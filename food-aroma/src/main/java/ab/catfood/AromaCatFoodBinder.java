...
@Service
@Import({AromaCatFoodProvider.class})
public class AromaCatFoodBinder
  ...
  final AromaCatFoodMessageSupport messageSupport;

  @Autowired
  public AromaCatFoodBinder(AromaCatFoodProvider provisioningProvider, AromaCatFoodMessageSupport messageSupport) {
  ...
  protected MessageHandler createProducerMessageHandler(ProducerDestination destination, ProducerProperties producerProperties, MessageChannel errorChannel) {
    messageSupport.setProducerDestination(destination);
  ...
  protected MessageProducer createConsumerEndpoint(ConsumerDestination destination, String group, ConsumerProperties properties) {
    messageSupport.setConsumerDestination(destination);
    return messageSupport;
  }
}
